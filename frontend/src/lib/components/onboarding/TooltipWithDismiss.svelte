<script lang="ts">
	...
	onMount(async () => {
		...
		try {
			const { data } = await api.get(`/admin/preferences/tooltip.${tooltipId}/completed`);
			if (data.completed) {
				return; // Don't show dismissed tooltip
			}
		} catch (error) {
			// Check localStorage as fallback
			if (typeof window !== 'undefined') {
				const dismissed = localStorage.getItem(`tooltip.${tooltipId}.dismissed`);
				if (dismissed) {
					return;
				}
			}
	...
	async function dismiss(): Promise<void> {
		...
		try {
			loading = true;
			await api.post(`/admin/preferences/tooltip.${tooltipId}/complete?completed=true`);
			
			// Also save to localStorage
			if (typeof window !== 'undefined') {
				localStorage.setItem(`tooltip.${tooltipId}.dismissed`, 'true');
			}
		} catch (error) {
