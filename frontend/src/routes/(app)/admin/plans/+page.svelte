                ...
                <Card.Root>
                    ...
                    <Card.Content class="pt-4 space-y-4">
                        ...
                        <div class="pt-4 border-t border-border/50 space-y-2">
                            ...
                            {#each [{ label: "Weight Profiles", value: plan.weightProfilesEnabled }, { label: "Signed Tokens", value: plan.signedTokenEnabled }, { label: "SSO Integration", value: plan.ssoEnabled }, { label: "Custom Branding", value: plan.customBrandingEnabled }, { label: "Device Fingerprint", value: plan.deviceFingerprintEnabled }, { label: "API Access", value: plan.apiAccessEnabled }] as toggle}
                                <div
                                    class="flex justify-between text-sm items-center"
                                >
                                    <span class="text-muted-foreground"
            ...
            <div class="overflow-y-auto px-6 py-2 flex-1">
                ...
                >
                    ...
                    <div class="pt-4 border-t border-border space-y-4">
                        ...
                        <p class="text-xs text-muted-foreground mb-4">
                            Enable or disable premium features for this plan
                            tier.
                        </p>

                        <div class="grid sm:grid-cols-2 gap-4">
                            {#each [{ label: "Weight Profiles", key: "weightProfilesEnabled" }, { label: "Signed Launch Tokens", key: "signedTokenEnabled" }, { label: "SSO Integration", key: "ssoEnabled" }, { label: "Custom Branding", key: "customBrandingEnabled" }, { label: "Device Fingerprint Dedup", key: "deviceFingerprintEnabled" }, { label: "API Access", key: "apiAccessEnabled" }] as toggle}
                                <div
                                    class="flex items-center justify-between gap-2 rounded-lg border border-border/50 px-3 py-2"
                                >
                                    <Label
                                        for={`ft-${toggle.key}`}
                                    ...
                                    <Switch
                                        id={`ft-${toggle.key}`}
                                        checked={(
                                            editingPlan as Record<string, any>
                                        )?.[toggle.key] ?? false}
                                        onCheckedChange={(v) => {
                                            if (editingPlan)
                                                (
                                                    editingPlan as Record<
                                                        string,
                                                        any
                                                    >
                                                )[toggle.key] = v;
